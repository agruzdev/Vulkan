...
public:
    ...
    Sample_03_Window(ApiWithoutSecrets::OS::WindowParameters window, uint32_t width, uint32_t height)
    {
        ...
        */
        ...
        //mVertexShader   = LoadShader(QUOTE(SHADERS_DIR) "/spv/04.vert.spv");
        mVertexShader = LoadShader(QUOTE(SHADERS_DIR) "/spv/vert.spv");
        std::cout << "OK" << std::endl;

        std::cout << "Loading fragment shader... ";
        //mFragmentShader = LoadShader(QUOTE(SHADERS_DIR) "/spv/04.frag.spv");
