    ...
    Sample_03_Window(ApiWithoutSecrets::OS::WindowParameters window, uint32_t width, uint32_t height)
    {
        ...
        */
...

        {
            ...
            bufferInfo.setUsage(vk::BufferUsageFlagBits::eVertexBuffer | vk::BufferUsageFlagBits::eStorageBuffer);
...

        {
            ...
            bufferInfo.setUsage(vk::BufferUsageFlagBits::eIndexBuffer | vk::BufferUsageFlagBits::eStorageBuffer);
            bufferInfo.setSharingMode(vk::SharingMode::eExclusive);
            mIndexesBuffer = MakeHolder(mDevice->createBuffer(bufferInfo, nullptr, mVkDispatcher), [this](vk::Buffer & buffer) { mDevice->destroyBuffer(buffer, nullptr, mVkDispatcher); });
        }
        ...
        // Upload vertex data
        {
            ...
            void* devicePtr = mDevice->mapMemory(mVertexMemory, 0, vertexBufferSize, vk::MemoryMapFlags{}, mVkDispatcher);
        ...
        // Upload index data
        {
            vk::MemoryRequirements indexMemoryRequirements = mDevice->getBufferMemoryRequirements(mIndexesBuffer, mVkDispatcher);
            ...
            for (uint32_t i = 0; i < memroProperties.memoryTypeCount; ++i) {
                if ((indexMemoryRequirements.memoryTypeBits & (1 << i)) &&
                    ...
                    allocateInfo.setMemoryTypeIndex(i);
                    mIndexesMemory = MakeHolder(mDevice->allocateMemory(allocateInfo, nullptr, mVkDispatcher), [this](vk::DeviceMemory & memory) { mDevice->freeMemory(memory, nullptr, mVkDispatcher); });
                    break;
                }
            }
            if (!mIndexesMemory) {
            ...
            mDevice->bindBufferMemory(mIndexesBuffer, mIndexesMemory, 0, mVkDispatcher);

            void* devicePtr = mDevice->mapMemory(mIndexesMemory, 0, indexesBufferSize, vk::MemoryMapFlags{}, mVkDispatcher);
            if (devicePtr == nullptr) {
                throw std::runtime_error("Failed to map memory for vertex buffer");
            }
            ...
            mDevice->unmapMemory(mIndexesMemory, mVkDispatcher);
        }

        // Bottom-level AS

        vk::GeometryTrianglesNV meshTriangles{};
        meshTriangles.setVertexCount(static_cast<uint32_t>(mesh.vertexes.size()));
        meshTriangles.setVertexData(mVertexBuffer);
        meshTriangles.setVertexOffset(offsetof(VertexData, position));
        meshTriangles.setVertexFormat(vk::Format::eR32G32B32Sfloat);
        meshTriangles.setVertexStride(sizeof(VertexData));
        meshTriangles.setIndexCount(static_cast<uint32_t>(mesh.indexes.size()));
        meshTriangles.setIndexData(mIndexesBuffer);
        meshTriangles.setIndexOffset(0);
        meshTriangles.setIndexType(vk::IndexType::eUint32);

        vk::GeometryAABBNV meshAabs{};
        meshAabs.setNumAABBs(0);

        vk::GeometryDataNV meshData{};
        meshData.setTriangles(meshTriangles);
        meshData.setAabbs(meshAabs);

        vk::GeometryNV meshGeometry{};
        meshGeometry.setGeometry(meshData);
        meshGeometry.setGeometryType(vk::GeometryTypeNV::eTriangles);
        meshGeometry.setFlags(vk::GeometryFlagBitsNV::eOpaque);
        ...
        accelerationStructureInfoBottom.setGeometryCount(1);
        accelerationStructureInfoBottom.setPGeometries(&meshGeometry);
        ...
        {
            auto code = GetBinaryShaderFromSourceFile(QUOTE(SHADERS_DIR) "/glsl/17.rgen");
            if (code.empty()) {
                throw std::runtime_error("LoadShader: Failed to read shader file!");
            }
            vk::ShaderModuleCreateInfo shaderInfo;
        ...
        {
            auto code = GetBinaryShaderFromSourceFile(QUOTE(SHADERS_DIR) "/glsl/17.rmiss");
            if (code.empty()) {
                throw std::runtime_error("LoadShader: Failed to read shader file!");
            }
            vk::ShaderModuleCreateInfo shaderInfo;
        ...
        {
            auto code = GetBinaryShaderFromSourceFile(QUOTE(SHADERS_DIR) "/glsl/17.rchit");
            if (code.empty()) {
                throw std::runtime_error("LoadShader: Failed to read shader file!");
            }
            vk::ShaderModuleCreateInfo shaderInfo;
        ...
        std::array<vk::DescriptorSetLayoutBinding, 5> descriptosBindings;
        ...
        descriptosBindings[0].setStageFlags(vk::ShaderStageFlagBits::eRaygenNV);
        ...
        descriptosBindings[2].setStageFlags(vk::ShaderStageFlagBits::eClosestHitNV);

        descriptosBindings[3].setDescriptorType(vk::DescriptorType::eStorageBuffer);
        descriptosBindings[3].setBinding(3);
        descriptosBindings[3].setDescriptorCount(1);
        descriptosBindings[3].setStageFlags(vk::ShaderStageFlagBits::eClosestHitNV);

        descriptosBindings[4].setDescriptorType(vk::DescriptorType::eUniformBuffer);
        descriptosBindings[4].setBinding(4);
        descriptosBindings[4].setDescriptorCount(1);
        descriptosBindings[4].setStageFlags(vk::ShaderStageFlagBits::eRaygenNV | vk::ShaderStageFlagBits::eClosestHitNV);
        ...
        descriptorPoolSizes[2].setDescriptorCount(static_cast<uint32_t>(2 * mRenderingResources.size()));
        ...
        shaderStages[1].setStage(vk::ShaderStageFlagBits::eClosestHitNV);
        shaderStages[1].setModule(mRayCloseHitShader);
        shaderStages[1].setPName("main");

        shaderStages[2].setStage(vk::ShaderStageFlagBits::eMissNV);
        shaderStages[2].setModule(mRayMissShader);
        ...
        shaderGroups[1].setType(vk::RayTracingShaderGroupTypeNV::eTrianglesHitGroup);
        shaderGroups[1].setGeneralShader(VK_SHADER_UNUSED_NV);
        shaderGroups[1].setIntersectionShader(VK_SHADER_UNUSED_NV);
        shaderGroups[1].setAnyHitShader(VK_SHADER_UNUSED_NV);
        shaderGroups[1].setClosestHitShader(1);

        shaderGroups[2].setType(vk::RayTracingShaderGroupTypeNV::eGeneral);
        shaderGroups[2].setGeneralShader(2);
        ...
        for (size_t i = 0; i < mRenderingResources.size(); ++i) {
            ...
            indexBufferInfo.setBuffer(mIndexesBuffer);
            indexBufferInfo.setOffset(0);
            indexBufferInfo.setRange(VK_WHOLE_SIZE);

            descriptorWrites[3].setDescriptorCount(1);
            descriptorWrites[3].setDescriptorType(vk::DescriptorType::eStorageBuffer);
            descriptorWrites[3].setDstSet(mRenderingResources[i].descriptorSet);
            descriptorWrites[3].setDstBinding(3);
            descriptorWrites[3].setPBufferInfo(&indexBufferInfo);
            ...
            descriptorWrites[4].setDstSet(mRenderingResources[i].descriptorSet);
            descriptorWrites[4].setDstBinding(4);
            descriptorWrites[4].setPBufferInfo(&cameraBufferInfo);
            ...
            // Draw
            {
                ...
                command->traceRaysNV(mShaderBindingTable, 0, mShaderBindingTable, 2 * rayTacingProps.shaderGroupHandleSize, rayTacingProps.shaderGroupHandleSize,
                    mShaderBindingTable, rayTacingProps.shaderGroupHandleSize, rayTacingProps.shaderGroupHandleSize, nullptr, 0, 0, 
...
int main()
{
    try {
        ...
        // Window creation
        if (!window.Create("17 - Ray Tracing", 512, 512)) {
            return -1;
        }
